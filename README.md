...
- [Installing](#installing)
  ...
  - [nbgrader plugin](#nbgrader-plugin)
    - [`Exchange History` extension](#exchange-history-extension)
...
### `Exchange History` extension

In version 2.0, nbexchange adds a new extension: `Exchange History`

This plugin shows all the courses a user is subscribed to, the assignments for each course, and the
[appropriate] actions for each assignment.

- All users see `release` actions, irrespective of the user who did the release
- Students only see actions _they_ perfomed
- Instructors see _all_ actions for the assignments of the courses they are instructors for.

To install & enable the extension in a notebook, add the following after enable `nbgrader`:

```
RUN jupyter nbextension install --sys-prefix --py nbexchange \
    && jupyter nbextension enable --sys-prefix  nbexchange_history/main --section=tree \
    && jupyter serverextension enable --sys-prefix nbexchange.server_extensions.nbexchange_history
```
