...
**GET**: gets a list of courses, assignments, and actions
    ...
    {"success": True,
     "value": {
        $course_id: {
            ...
            course_code: Str,
            course_title: Str,
            role: {  // set of roles seen on this course, so we can count them
                $role: 1,
                $role2: 1,
                ..
                },
            user_id: {  // set of users seen on this course, so we can count them
                $user_id: 1,
                $user_id2: 1,
                ..
                },
            isInstructor: Bool, // True if this.user is an instructor
            assignments: [
                {
                    assignment_code: Str,
                    assignment_id: Int
                    ...
                    action_summary: { // sum of actions for this assignment
                        $action: Int
                        $action2: Int
                    }
                },
...
(If the user is not authenticated, or not subscribed to any courses, an empty dict is returned.)