...
from nbexchange.plugin.util import get_files, get_directory_structure

from nbexchange.plugin.filestore import LocalFileStore

from nbexchange.plugin.filestore import RemoteFileStore
...
""",
    ...
    def __init__(self, coursedir=None, authenticator=None, **kwargs):
        super(Exchange, self).__init__(coursedir, authenticator, **kwargs)
        self.course_store = LocalFileStore(
            coursedir.root if coursedir is not None else "",
            coursedir.directory_structure
            if coursedir is not None
            else os.path.join("{nbgrader_step}", "{student_id}", "{assignment_id}"),
        )

        self.assignment_store = LocalFileStore(
            self.assignment_dir,
            os.path.join("{course_id}", "{assignment_id}")
            if self.path_includes_course
            else "{assignment_id}",
        )
        self.remote_store = RemoteFileStore(
            self.assignment_dir,
            os.path.join("{course_id}", "{assignment_id}")
            if self.path_includes_course
            else "{assignment_id}",
