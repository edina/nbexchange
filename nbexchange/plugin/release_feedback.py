import datetime
import glob
import io
import json
import shutil
import re

import nbgrader.exchange.abc as abc
import os
import tempfile

from urllib.parse import quote_plus
from dateutil import parser

from nbgrader.utils import notebook_hash, make_unique_key

from .exchange import Exchange


class ExchangeReleaseFeedback(abc.ExchangeReleaseFeedback, Exchange):
    """
    This can get called for a named "student", or ron for all "students". For
      nbexchange, we want to 1 upload action per "student", so if
      self.coursedir.student_id is not set, we need to figure out
      who that are, adn loop over them. See
    self.src is where the instructors assignmetn file(s) are stored.
      We don't actually use this, because we probably need make a list of
      students to loop over (see in "copy_files")
    self.dest is moot, we're uploading to the exchange
    self.notebooks is the list of notebooks in the released assignment
      (which is then used everywhere to compare what should be submitted
      or included in feedback)
    self.upload pushes a gzipped file to the exchange
    """

    src_path = None
    dest_path = None
    notebooks = None

    # where the feedback files files are to be found
    def init_src(self):
        student_id = self.coursedir.student_id if self.coursedir.student_id else "*"
        self.src_path = self.coursedir.format_path(
            self.coursedir.feedback_directory, student_id, self.coursedir.assignment_id
        )

    # where in the user tree
    def init_dest(self):
        pass

    def copy_if_missing(self, src, dest, ignore=None):
        pass

    def do_copy(self, src, dest):
        pass

    def tar_source(self, student_id):
        """
        Creates an in-memory tar-ball of all the feedback files
        for the student
        """
        if not student_id:
            return

        import tarfile

        tar_file = io.BytesIO()

        with tarfile.open(fileobj=tar_file, mode="w:gz") as tar_handle:
            # needs to be modded to honour 'path_includes_course'
            fb_path = os.path.join(
                self.coursedir.feedback_directory,
                student_id,
                self.coursedir.assignment_id,
            )
            html_files = glob.glob(os.path.join(fb_path, "*.html"))
            for html_file in html_files:
                tar_handle.add(html_file, arcname=".")
        tar_file.seek(0)
        return tar_file.read()

    def copy_files(self):
        if self.coursedir.student_id_exclude:
            exclude_students = set(self.coursedir.student_id_exclude.split(","))
        else:
            exclude_students = set()

        # Calculate the list of students we need to work with.
        # Note, the nbgrader path strudture is
        #   .../feedback/<student>/<assignment>/ - so we can't just take
        #   the list of <student>s - they may not all be relevant!
        # Note that self.src_path has done the 'path_includes_course' thing
        html_files = glob.glob(os.path.join(self.src_path, "*.html"))
        students = set()
        for html_file in html_files:
            regexp = re.escape(os.path.sep).join(
                [
                    os.path.normpath(
                        self.coursedir.format_path(
                            self.coursedir.feedback_directory,
                            "(?P<student_id>.*)",
                            self.coursedir.assignment_id,
                            escape=True,
                        )
                    ),
                ]
            )

            m = re.match(regexp, html_file)
            if m is None:
                msg = "Could not match '%s' with regexp '%s'" % (html_file, regexp)
                self.log.error(msg)
                continue
            student_id = m["student_id"]
            if student_id in exclude_students:
                self.log.debug("Skipping student '{}'".format(student_id))
                continue
            students.add(student_id)

        # Loop through the list of students, uploading a .gz file of the .html files
        for student in students:

            # Grab files from hard drive
            file = self.tar_source(student)
            self.upload(file, student)

        # html_files = glob.glob(os.path.join(self.src_path, "*.html"))
        # for html_file in html_files:
        #     regexp = re.escape(os.path.sep).join(
        #         [
        #             os.path.normpath(
        #                 self.coursedir.format_path(
        #                     self.coursedir.feedback_directory,
        #                     "(?P<student_id>.*)",
        #                     self.coursedir.assignment_id,
        #                     escape=True,
        #                 )
        #             ),
        #             "(?P<notebook_id>.*).html",
        #         ]
        #     )

        #     m = re.match(regexp, html_file)
        #     if m is None:
        #         msg = "Could not match '%s' with regexp '%s'" % (html_file, regexp)
        #         self.log.error(msg)
        #         continue

        #     gd = m.groupdict()
        #     self.log.info(f"release feedback groupdict '{gd}'")
        #     student_id = gd["student_id"]
        #     notebook_id = gd["notebook_id"]
        #     if student_id in exclude_students:
        #         self.log.debug("Skipping student '{}'".format(student_id))
        #         continue

        #     feedback_dir = os.path.split(html_file)[0]
        #     submission_dir = self.coursedir.format_path(
        #         self.coursedir.submitted_directory,
        #         student_id,
        #         self.coursedir.assignment_id,
        #     )

        #     timestamp = open(os.path.join(feedback_dir, "timestamp.txt")).read().strip()
        #     nbfile = os.path.join(submission_dir, "{}.ipynb".format(notebook_id))
        #     unique_key = make_unique_key(
        #         self.course_id,
        #         self.coursedir.assignment_id,
        #         notebook_id,
        #         student_id,
        #         timestamp,
        #     )

        #     self.log.debug("Unique key is: {}".format(unique_key))
        #     checksum = notebook_hash(nbfile, unique_key)

        #     timestamp = parser.parse(timestamp).strftime(self.timestamp_format).strip()

        #     self.log.info(
        #         "Releasing feedback for student '{}' on assignment '{}/{}/{}' ({})".format(
        #             student_id,
        #             self.coursedir.course_id,
        #             self.coursedir.assignment_id,
        #             notebook_id,
        #             timestamp,
        #         )
        #     )

        #     self.upload(
        #         html_file,
        #         self.coursedir.assignment_id,
        #         student_id,
        #         notebook_id,
        #         timestamp,
        #         checksum,
        #     )

    # def upload(self, html_file, assignment_id, student, notebook, timestamp, checksum):

    #     with open(html_file) as feedback_file:
    #         files = {"feedback": ("feedback.html", feedback_file.read())}

    #     url = (
    #         f"feedback?course_id={quote_plus(self.course_id)}"
    #         f"&assignment_id={quote_plus(assignment_id)}"
    #         f"&notebook={quote_plus(notebook)}"
    #         f"&student={quote_plus(student)}"
    #         f"&timestamp={quote_plus(timestamp)}"
    #         f"&checksum={quote_plus(checksum)}"
    #     )

    #     r = self.api_request(url, method="POST", files=files)

    #     self.log.debug(f"Got back {r.status_code} after feedback upload")

    #     try:
    #         data = r.json()
    #     except json.decoder.JSONDecodeError:
    #         self.fail(r.text)

    #     if not data["success"]:
    #         self.fail(data["note"])

    #     self.log.info("Successfully uploaded feedback for assignment.")

    def upload(self, file, student_id):
        files = {"feedback": ("feedback.tar.gz", file)}

        url = f"feedback?course_id={quote_plus(self.course_id)}&assignment_id={quote_plus(self.coursedir.assignment_id)}&student={quote_plus(student_id)}"

        r = self.api_request(url, method="POST", files=files)
        self.log.debug(f"Got back {r.status_code} after file upload")

        try:
            data = r.json()
        except json.decoder.JSONDecodeError:
            self.fail(r.text)

        if not data["success"]:
            self.fail(data["note"])

        self.log.info("Successfully uploaded released feedback.")
