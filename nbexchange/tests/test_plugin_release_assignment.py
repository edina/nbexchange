...
def test_release_assignment_normal(plugin_config, tmpdir):
    ...
    def api_request(*args, **kwargs):
        assert args[0] == (
            f"assignment?course_id=no_course"
...
def test_release_assignment_several_normal(plugin_config, tmpdir):
    ...
    os.makedirs(os.path.join(plugin_config.CourseDirectory.release_directory, "assign_1"), exist_ok=True)
    copyfile(notebook1_filename,
             os.path.join(plugin_config.CourseDirectory.release_directory, "assign_1", "release1.ipynb"))
    with open(os.path.join(plugin_config.CourseDirectory.release_directory, "assign_1", "timestamp.txt"), "w") as fp:
        fp.write("2020-01-01 00:00.0 UTC")
    ...
    copyfile(notebook2_filename,
             os.path.join(plugin_config.CourseDirectory.release_directory, "assign_1", "release2.ipynb"))
...
def test_release_assignment_fail(plugin_config, tmpdir):
    ...
    os.makedirs(os.path.join(plugin_config.CourseDirectory.release_directory,  "assign_1"), exist_ok=True)
    copyfile(notebook1_filename,
             os.path.join(plugin_config.CourseDirectory.release_directory,  "assign_1", "feedback.ipynb"))
    with open(os.path.join(plugin_config.CourseDirectory.release_directory, "assign_1", "timestamp.txt"), "w") as fp:
        fp.write("2020-01-01 00:00.0 UTC")
